  ...
  this.cancelrebill = {
    post: function(params, callback) {
      var schema = {
        ...
        item_id: {
          description:
            'Unique Rebilling Identifier of the Rebill for cancellation. Returned in Notifications or available via the Gateway.',
          type: 'number',
          maxLength: 12,
          required: true,
        },
  ...
  this.search = {
    post: function(params, callback) {
      var schema = {
        ...
        date: {
          description:
            'Order Creation date returns all orders created on this date. In the format YYYY-MM-DD This field is mandatory if the order_id,trans_id, rebill_id and reference fields are not present.',
          type: 'string',
          format: 'date',
        },
        include_risk_data: {
          description:
            'Allows Risk Data from all Risk Providers queried during this transaction to be retrieved along with the transaction details.',
          type: 'number',
          maxLength: 1,
          default: null,
        },
      ...
      var dateRequired =
        _.filter(['order_id', 'trans_id', 'rebill_id'], function(n) {
